...
## D�ploiement s�curis� (GitHub Pages + Actions)

Le workflow `.github/workflows/deploy.yml` construit et publie l'app sur GitHub Pages � chaque
push sur `main` (ou manuellement via *Actions ? Run workflow*).

### Mise en place
1. **Secrets du d�p�t** (Settings ? Secrets and variables ? Actions) :
   - `SUPABASE_URL` : `https://<projet>.supabase.co`
   - `SUPABASE_ANON_KEY` : la cl� **publishable/anon** (publique par nature)
   Le workflow g�n�re `public/config.json` � la vol�e : **aucune cl� n'est versionn�e**.
2. **Pages** : Settings ? Pages ? *Source* = **GitHub Actions**.
3. Pousser sur `main` ? l'app est publi�e sur `https://<user>.github.io/<repo>/`.

### Mod�le de s�curit�
- **La s�curit� r�elle vient de PostgreSQL (RLS)**, pas de GitHub Pages (h�bergement statique).
- La cl� publishable est **publique** : elle est de toute fa�on envoy�e au navigateur.
  Elle n'autorise que ce que les policies RLS permettent.
- **Lecture publique**, **�criture r�serv�e aux utilisateurs pr�sents dans `admin_users`**
  (v�rifi� : `anon` peut lire, `anon` ne peut pas �crire).
- L'admin (`/admin`) est prot�g� par **Supabase Auth** + guard Angular + RLS (la base refuse
  toute �criture sans session valide, m�me si on contourne le guard).
- Le **mot de passe PostgreSQL** et la cl� **service_role** ne doivent JAMAIS �tre dans le front
  ni dans le repo. Les scripts `db:migrate` / `db:seed` utilisent `DATABASE_URL` en local uniquement.
- **CSP** stricte dans `src/index.html` : seules les sources n�cessaires sont autoris�es
  (Supabase, Wikimedia, OSM, polices, Leaflet). `object-src 'none'`.
- HTTPS et en-t�tes de base fournis par GitHub Pages.

## Acc�s priv� & gestion des comptes

Depuis la v31, **l'ensemble du site n�cessite une connexion** (plus seulement l'admin).
- Sans session ? redirection vers `/login`.
- L'admin (`/admin`) exige en plus le r�le **administrateur** (table `admin_users`).
- **Pas d'inscription publique** : seuls les comptes cr��s par un admin peuvent se connecter.

Cr�er/g�rer les comptes : Admin ? **Utilisateurs** (email, mot de passe = 8, r�le admin).
Techniquement, la cr�ation passe par des fonctions PostgreSQL `security definer`
(`admin_create_user`, `admin_list_users`, `admin_set_admin`, `admin_delete_user`) qui v�rifient
`is_admin()` : **la cl� service_role n'est jamais expos�e au navigateur**.
