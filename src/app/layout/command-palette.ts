...
const PAGES: Result[] = [
  { key: 'p-dashboard', icon: 'home', label: 'Dashboard', kind: 'Page', link: '/dashboard' },
  { key: 'p-today', icon: 'pin', label: "Aujourd'hui", kind: 'Page', link: '/today' },
  { key: 'p-itinerary', icon: 'route', label: 'Itinéraire', kind: 'Page', link: '/itinerary' },
  { key: 'p-timeline', icon: 'calendar', label: 'Timeline', kind: 'Page', link: '/timeline' },
  { key: 'p-sheets', icon: 'book', label: 'Fiches Voyage', kind: 'Page', link: '/sheets' },
  { key: 'p-stats', icon: 'chart', label: 'Statistiques', kind: 'Page', link: '/statistics' },
  { key: 'p-packing', icon: 'backpack', label: 'Packing List', kind: 'Page', link: '/packing' },
  { key: 'p-checklist', icon: 'check', label: 'Check-list départ', kind: 'Page', link: '/checklist' },
  { key: 'p-logistics', icon: 'train', label: 'Logistique Japon', kind: 'Page', link: '/logistics' },
  { key: 'p-restos', icon: 'utensils', label: 'Restos & Souvenirs', kind: 'Page', link: '/restaurants' },
  { key: 'p-discover', icon: 'compass', label: 'Découvrir', kind: 'Page', link: '/discover' },
  { key: 'p-phrases', icon: 'message', label: 'Phrasebook', kind: 'Page', link: '/phrasebook' },
  { key: 'p-culture', icon: 'flag', label: 'Agenda culturel', kind: 'Page', link: '/culture' },
  { key: 'p-moodboard', icon: 'image', label: 'Moodboard', kind: 'Page', link: '/moodboard' },
  { key: 'p-photos', icon: 'camera', label: 'Photos', kind: 'Page', link: '/photos' },
  { key: 'p-weather', icon: 'sun', label: 'Météo & Saison', kind: 'Page', link: '/weather' },
  { key: 'p-japan', icon: 'info', label: 'Japon 101', kind: 'Page', link: '/japan-101' },
  { key: 'p-surprise', icon: 'dice', label: 'Surprise !', kind: 'Page', link: '/surprise' },
  { key: 'p-print', icon: 'printer', label: 'Impression', kind: 'Page', link: '/print' },
];
          ...
          <ul class="cmd-results">
            @for (r of results(); track r.key; let i = $index) {
              <li class="cmd-item" [class.active]="i === index()" (click)="select(r)" (mouseenter)="index.set(i)">
                <app-icon class="cmd-icon" [name]="r.icon" [size]="16" />
...
export class CommandPalette {
  ...
  readonly results = computed<Result[]>(() => {
    ...
    for (const d of this.content.destinations()) {
      all.push({ key: 'd-' + d.slug, icon: 'map', label: d.name, sub: d.name_jp ?? undefined, kind: 'Fiche', link: '/sheets', queryParams: { city: d.name } });
    }
    for (const p of this.content.content().phrases) {
      all.push({ key: 'ph-' + (p.id ?? p.fr), icon: 'message', label: p.fr, sub: p.jp ?? undefined, kind: 'Phrase', link: '/phrasebook' });
    }
    for (const r of this.content.restaurants()) {
      all.push({ key: 'r-' + (r.id ?? r.name), icon: 'utensils', label: r.name, sub: r.city ?? undefined, kind: 'Resto', link: '/restaurants' });
    }
